...
#ifndef ROTORS_CONTROL_GEOMETRIC_POSE_CONTROLLER_H
#define ROTORS_CONTROL_GEOMETRIC_POSE_CONTROLLER_H
...
// Default values for the PD gains.
...
class GeometricPoseControllerParameters {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  GeometricPoseControllerParameters()
...
class GeometricPoseController {
 public:
  GeometricPoseController();
  ~GeometricPoseController();
  void InitializeParameters();
  void CalculateDesiredWrench(Eigen::Vector3d* force_B, Eigen::Vector3d* torque_B) const;

  void SetOdometry(const EigenOdometry& odometry);
  void SetTrajectoryPoint(
    const mav_msgs::EigenTrajectoryPoint& command_trajectory);

  GeometricPoseControllerParameters controller_parameters_;
 ...
 private:
  ...
  Eigen::Vector3d normalized_position_gain_;
  Eigen::Vector3d normalized_velocity_gain_;
...
#endif // ROTORS_CONTROL_GEOMETRIC_POSE_CONTROLLER_H
